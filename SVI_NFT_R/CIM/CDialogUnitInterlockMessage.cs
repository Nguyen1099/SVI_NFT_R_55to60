...
namespace SVI_NFT_R
{
    public partial class CDialogUnitInterlockMessage : CFormCommon, CFormInterface
    {
        /// <summary>
        /// 임의 인터락 데이터 리스트
        /// </summary>
        ...
        private const string BACKUP_FILE_NAME = @".\cache\UnitInterlockMessage.bak";
        ...
        /// <returns></returns>
        public CDialogUnitInterlockMessage(CDocument objDocument)
        {
            InitializeComponent();
        ...
        /// <param name="e"></param>
        private void CDialogUnitInterlockMessage_Load(object sender, EventArgs e)
        {
            Initialize();
        }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void CDialogUnitInterlockMessage_FormClosed(object sender, FormClosedEventArgs e)
        {
            DeInitialize();
        }
        ...
        public bool InitializeDialog()
        {
            ...
            {
                // 알람 리스트 그리드 뷰 초기화
                string[] strInterlockList = { EInterlockListColumn.DATETIME.ToString(), EInterlockListColumn.UNITID.ToString(), EInterlockListColumn.INTERLOCKID.ToString(), EInterlockListColumn.MESSAGE.ToString() };
                if (false == InitializeGridView(this.dataGridViewInterlock, strInterlockList)) break;
        ...
        private bool InitializeGridView(DataGridView objGridView, string[] strColumnName)
        {
            ...
            do
            {
                ...
                objGridView.Columns[1].Width = (int)(objGridView.Width * 0.14);
                objGridView.Columns[2].Width = (int)(objGridView.Width * 0.08);
                objGridView.Columns[3].Width = (int)(objGridView.Width * 0.64);
            ...
            do
            {
                ...
                lock (this.dataGridViewInterlock)
                {
                    ...
                    for (int iLoopInterlockList = 0; iLoopInterlockList < objInterlockList.Count; iLoopInterlockList++)
                    {
                        ...
                        strRowData[(int)EInterlockListColumn.UNITID] = objInterlockList[iLoopInterlockList].m_strUnitID;
        ...
        public void SetListData(DateTime time, string strUnitID, string strInterlockID, string strMessage)
        {
            do
            {
        ...
        private void BtnConfirm_Click(object sender, EventArgs e)
        {
            ...
            // Unit data 가져오기
            Dictionary<string, int> dicUnit = new Dictionary<string, int>();
            for (int iLoopCount = 0; iLoopCount < iItemCount; iLoopCount++)
            {
                string unitid = dataGridViewInterlock.Rows[iLoopCount].Cells[(int)EInterlockListColumn.UNITID].Value.ToString();
                if (dicUnit.ContainsKey(unitid) == false)
                {
                    dicUnit.Add(unitid, 1);
                }
                else
                {
                    dicUnit[unitid]++;
            ...
            foreach (string unitid in dicUnit.Keys)
            {
                UnitInterlockConfirmEvent objInterlockConfirmEvent = new UnitInterlockConfirmEvent();
                objInterlockConfirmEvent.EQPID = m_objDocument.m_objConfig.GetSystemParameter().strEquipmentID;

                objInterlockConfirmEvent.BODY.INTERLOCKS = new UnitInterlockConfirmEvent._UnitInterlockConfirmEvent.INTERLOCK[dicUnit[unitid]];
                objInterlockConfirmEvent.BODY.UNITID = unitid;
                objInterlockConfirmEvent.BODY.CELLID = " ";
                objInterlockConfirmEvent.BODY.PPID = m_objDocument.m_objConfig.GetSystemParameter().strPPID;
                objInterlockConfirmEvent.BODY.PRODUCTID = " ";
                objInterlockConfirmEvent.BODY.STEPID = " ";

                int nCount = 0;
                for (int iLoopCount = 0; iLoopCount < iItemCount; iLoopCount++)
                {
                    if (unitid == dataGridViewInterlock.Rows[iLoopCount].Cells[(int)EInterlockListColumn.UNITID].Value.ToString())
                    {
                        objInterlockConfirmEvent.BODY.INTERLOCKS[nCount].INTERLOCKID = dataGridViewInterlock.Rows[iLoopCount].Cells[(int)EInterlockListColumn.INTERLOCKID].Value.ToString();
                        objInterlockConfirmEvent.BODY.INTERLOCKS[nCount].MESSAGE = dataGridViewInterlock.Rows[iLoopCount].Cells[(int)EInterlockListColumn.MESSAGE].Value.ToString();
                        nCount++;
                    }
                }
                // CIM  Interlock 보고.
                m_objDocument.m_objProcessCIM.m_objCIMSendMessage.SetUnitInterlockConfirmEvent(objInterlockConfirmEvent);
            }
            ...
            // 타워 램프 히스토리 삭제
            m_objDocument.m_objProcessMain.m_objProcessTowerLamp.ClearHistory(CProcessTowerLamp.ETowerLamp.UnitInterlock);
        ...
        public class CModelInterlock
        {
            ...
            public CModelInterlock(DateTime objInterlockTime, string strUnitID, string strInterlockID, string strInterlockMessage)
            {
        ...
        private void CDialogUnitInterlockMessage_FormClosing(object sender, FormClosingEventArgs e)
        {
