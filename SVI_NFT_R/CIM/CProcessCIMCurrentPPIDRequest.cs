...
namespace SVI_NFT_R
{
    public class CProcessCIMCurrentPPIDRequest : CCIMAbstract
    {
        CurrentPPIDRequest m_objCurrentPPIDRequest;
        CurrentPPIDReply m_objCurrentPPIDReply;
        ...
        /// <returns></returns>
        public CProcessCIMCurrentPPIDRequest(CCIMDefine.structureCIMInitialize objCIMIntiialize)
        {
        ...
        protected override bool Initialize()
        {
            ...
            do
            {
                ...
                // 데이터 생성.
                m_objCurrentPPIDRequest = new CurrentPPIDRequest();
                m_objCurrentPPIDReply = new CurrentPPIDReply();
                m_objCurrentPPIDRequest.EQPID = m_strEquipmentID;
                m_objCurrentPPIDReply.EQPID = m_strEquipmentID;
        ...
        public void Communicator_OnReceiveRequest(object sender, ReqMsgInfo e)
        {
            try
            {
                ...
                foreach (XmlNode xn in xmlMsgnode)
                {
                    ...
                    if ("CURRENTPPIDREQUEST" == name)
                    {
                        m_objCurrentPPIDRequest.EQPID = eqpid;
                        m_objCurrentPPIDRequest.NAME = name;
                        m_objCurrentPPIDRequest.TRANSACTIONNO = transactionno;
                        m_bReceiveData = true;
        ...
        /// </summary>
        /// <returns></returns>
        public CurrentPPIDRequest GetReceivedData()
        {
            m_eStatus = CCIMDefine.EProcessState.STS_READY;
            return m_objCurrentPPIDRequest;
        }
        ...
        protected override bool DoProcess()
        {
            ...
            do
            {
                m_bReceiveData = false;
                // 현재 설비 상태 보내기.
                m_objCurrentPPIDReply.TRANSACTIONNO = m_objCurrentPPIDRequest.TRANSACTIONNO;
                m_objCurrentPPIDReply.BODY.PPID = m_objDocument.m_objConfig.GetModelParameter().strPPID;

                m_objCurrentPPIDReply.BODY.PPID_TYPE = "1";//m_objDocument.m_objConfig.GetModelParameter().strName;

                m_objDocument.m_objProcessCIM.m_objCIMSendMessage.SetCurrentPPIDReply(m_objCurrentPPIDReply);
                bResult = true;
            } while (false);
            return bResult;
        }
        ...
        {
            CProcessCIMCurrentPPIDRequest pThis = (CProcessCIMCurrentPPIDRequest)state;
