...
namespace SVI_NFT_R
{
    public partial class CFormTeachInspStage : CFormCommon, CFormInterface
    {
        private enum EMotor
        {
            STAGE_X = 0,
        ...
        private InspStage mInspStageManager;
        ...
        private readonly BatchMoveInformation[] mBatchMovePageInspStage = new BatchMoveInformation[]
        {
            new BatchMoveInformation(InspStage.EBatch.GrabProcess),
            null,
            null,
            null,
            null,

            new BatchMoveInformation(InspStage.EBatch.MoveLoadPosition),
            new BatchMoveInformation(InspStage.EBatch.MoveGrabP1Position),
            new BatchMoveInformation(InspStage.EBatch.MoveGrabP2Position),
            new BatchMoveInformation(InspStage.EBatch.MoveUnloadPosition),
            null,
        };
        ...
        private class BatchMoveInformation
        {
            ...
            /// 배치 동작 커맨드
            /// </summary>
            public InspStage.EBatch BatchCmd { get; private set; }

            public BatchMoveInformation(InspStage.EBatch batchCmd)
            {
                BatchCmd = batchCmd;
            }
        }
        ...
        /// <returns></returns>
        public CFormTeachInspStage(CDocument objDocument)
        {
            // 도큐먼트 포인트 받음
        ...
        /// <param name="e"></param>
        private void CFormTeachInspStage_Load(object sender, EventArgs e)
        {
            // 초기화
            Initialize();
        }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void CFormTeachInspStage_FormClosed(object sender, FormClosedEventArgs e)
        {
            // 해제
            DeInitialize();
        }
        ...
        public bool Initialize()
        {
            ...
            do
            {
                mInspStageManager = m_objDocument.m_objProcessMain.m_objProcessMotion.InspStage;
                // 폼 초기화
        ...
        public bool InitializeForm()
        {
            ...
            do
            {
                ...
                // 초기 모터 객체 설정
                m_objMotor = mInspStageManager.MotorStageY.Axis;
                mInterlock = mInspStageManager.MotorStageY.m_objInterlock;
                ...
                UcTeachVacuum1.Initialize(this, m_objDocument, mInspStageManager.Pickers[0].VacuumIndex, CDeviceIODefine.EDigitalInput.X_INSP_STAGE_P1_CELL_DETECT_SENSOR, GetIsBlowAutoOff, GetIsAnalogDataView);
                UcTeachVacuum2.Initialize(this, m_objDocument, mInspStageManager.Pickers[1].VacuumIndex, CDeviceIODefine.EDigitalInput.X_INSP_STAGE_P2_CELL_DETECT_SENSOR, GetIsBlowAutoOff, GetIsAnalogDataView);

                // 배치 이동 페이지 선택
                SetBatchMovePage(mBatchMovePageInspStage);
        ...
        public bool SetChangeLanguage()
        {
            ...
            do
            {
                ...
                var allBatchMovePages = mBatchMovePageInspStage;
        ...
        private void Timer_Tick(object sender, EventArgs e)
        {
            ...
            // 배치 동작 정보 표시
            {
                ...
                for (int i = 0; i < BATCH_PAGE_ITME_MAX_COUNT; i++)
                {
                    ...
                    if (mInspStageManager.BatchCommand == mBatchMovePage[startIndex + i].BatchCmd)
                    {
        ...
        private void SetBatchMoveExecuteItem(int index)
        {
            do
            {
                ...
                // Batch Move Execute
                mInspStageManager.BatchCommand = mBatchMovePage[targetIndex].BatchCmd;

                // Batch Move End Wait
                while (InspStage.EBatch.None != mInspStageManager.BatchCommand)
                {
        ...
        private void ShowMotorVisible(ImageButton selectedButton, out int selectedButtonIndex)
        {
            ...
            if (m_BtnMotor[(int)EMotor.STAGE_X] == selectedButton)
            {
                ...
                m_objMotor = mInspStageManager.MotorStageY.Axis;
                m_strSelectedMotor = mInspStageManager.MotorStageY.MotorIndex.ToString();
            }
