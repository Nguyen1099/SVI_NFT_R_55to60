...
namespace SVI_NFT_R
{
    public partial class CFormTeachOutFlip : CFormCommon, CFormInterface
    {
        private enum EMotor
        {
            FLIP_Z = 0,
        ...
        private OutFlip mOutFlipManager;
        ...
        private readonly BatchMoveInformation[] mBatchMovePageOutFlip = new BatchMoveInformation[]
        {
            new BatchMoveInformation(OutFlip.EBatch.SetZero),
            new BatchMoveInformation(OutFlip.EBatch.LoadWait),
            new BatchMoveInformation(OutFlip.EBatch.Unload),
            null,
            null,
        };
        ...
        private class BatchMoveInformation
        {
            ...
            /// 배치 동작 커맨드
            /// </summary>
            public OutFlip.EBatch BatchCmd { get; private set; }

            public BatchMoveInformation(OutFlip.EBatch batchCmd)
            {
                BatchCmd = batchCmd;
            }
        }
        ...
        /// <returns></returns>
        public CFormTeachOutFlip(CDocument objDocument)
        {
            // 도큐먼트 포인트 받음
        ...
        /// <param name="e"></param>
        private void CFormTeachOutFlip_Load(object sender, EventArgs e)
        {
            // 초기화
            Initialize();
        }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void CFormTeachOutFlip_FormClosed(object sender, FormClosedEventArgs e)
        {
            // 해제
            DeInitialize();
        }
        ...
        public bool Initialize()
        {
            ...
            do
            {
                mOutFlipManager = m_objDocument.m_objProcessMain.m_objProcessMotion.OutFlip;
        ...
        public bool InitializeForm()
        {
            ...
            do
            {
                ...
                UcTeachVacuum1.Initialize(this, m_objDocument, mOutFlipManager.Pickers[0].VacuumIndex, GetIsBlowAutoOff, GetIsAnalogDataView);
                UcTeachVacuum2.Initialize(this, m_objDocument, mOutFlipManager.Pickers[1].VacuumIndex, GetIsBlowAutoOff, GetIsAnalogDataView);
                UcTeachVacuum3.Initialize(this, m_objDocument, mOutFlipManager.Pickers[2].VacuumIndex, GetIsBlowAutoOff, GetIsAnalogDataView);
                UcTeachVacuum4.Initialize(this, m_objDocument, mOutFlipManager.Pickers[3].VacuumIndex, GetIsBlowAutoOff, GetIsAnalogDataView);

                // 배치 이동 페이지 선택
                SetBatchMovePage(mBatchMovePageOutFlip);

                // 첫 번째 모터 선택
                BtnMotorFlipR1.PerformClick();
        ...
        {
            BtnOutConveyorSensor1.Tag = new BindingSelection(m_objDocument, BtnOutConveyorSensor1, null, (sender) => mOutFlipManager.Sensor.IsCellDetectSensor1);
            BtnOutConveyorSensor2.Tag = new BindingSelection(m_objDocument, BtnOutConveyorSensor2, null, (sender) => mOutFlipManager.Sensor.IsCellDetectSensor2);
            BtnOutConveyorBlockedSensor.Tag = new BindingSelection(m_objDocument, BtnOutConveyorBlockedSensor, null, (sender) => mOutFlipManager.Sensor.IsConveyorBlockedSensor);
        ...
        private void SetResourceControl()
        {
            ...
            SetButtonEnable(BtnMotorFlipR1, true);
            SetButtonEnable(BtnMotorFlipR2, true);
            SetButtonEnable(BtnMotorFlipZ, true);
            SetButtonEnable(BtnMotorConveyorX, true);
        }
        ...
        public bool SetChangeLanguage()
        {
            ...
            do
            {
                ...
                SetButtonChangeLanguage(BtnMotorFlipR1);
                SetButtonChangeLanguage(BtnMotorFlipR2);
                SetButtonChangeLanguage(BtnMotorFlipZ);
                SetButtonChangeLanguage(BtnMotorConveyorX);
                ...
                SetButtonChangeLanguage(BtnOutConveyorAutoMode);
                SetButtonChangeLanguage(BtnOutConveyorManualMode);
                SetButtonChangeLanguage(BtnOutConveyorSensor1);
                SetButtonChangeLanguage(BtnOutConveyorSensor2);
                SetButtonChangeLanguage(BtnOutConveyorBlockedSensor);

                var allBatchMovePages = mBatchMovePageOutFlip;
        ...
        private void Timer_Tick(object sender, EventArgs e)
        {
            ...
            // 배치 동작 정보 표시
            {
                ...
                for (int i = 0; i < BATCH_PAGE_ITME_MAX_COUNT; i++)
                {
                    ...
                    if (mOutFlipManager.BatchCommand == mBatchMovePage[startIndex + i].BatchCmd)
                    {
            ...
            foreach (var item in mBindingSelectionControls)
            {
                ...
                SpeedButton speedButton = item.Sender as SpeedButton;
                if (speedButton != null)
                {
                    SetButtonBackColor(speedButton, (item.IsSelected) ? m_colorInputOn : m_colorInputOff);
                }
            }

            // 아웃 컨베이어 제어 모드 표시
            {
                SetButtonBackColor(BtnOutConveyorAutoMode, (true == mOutFlipManager.IsOutConveyorAutoMode) ? m_colorOn : m_colorNormal, (true == mOutFlipManager.IsOutConveyorAutoMode) ? true : false);
                SetButtonBackColor(BtnOutConveyorManualMode, (false == mOutFlipManager.IsOutConveyorAutoMode) ? m_colorOn : m_colorNormal, (false == mOutFlipManager.IsOutConveyorAutoMode) ? true : false);
            }
        }
        ...
        private void SetBatchMoveExecuteItem(int index)
        {
            do
            {
                ...
                // Batch Move Execute
                mOutFlipManager.BatchCommand = mBatchMovePage[targetIndex].BatchCmd;

                // Batch Move End Wait
                while (OutFlip.EBatch.None != mOutFlipManager.BatchCommand)
                {
        ...
        private void ShowMotorVisible(ImageButton selectedButton, out int selectedButtonIndex)
        {
            ...
            if (m_BtnMotor[(int)EMotor.FLIP_R1] == selectedButton)
            {
                selectedButtonIndex = (int)EMotor.FLIP_R1;
                PnlContent.Visible = true;
                isMotorSelected = true;
                SetBatchMovePage(mBatchMovePageOutFlip);
                mUnit = CDefine.UNIT_ANGULAR;
                m_objMotor = mOutFlipManager.MotorRs[0].Axis;
                m_strSelectedMotor = mOutFlipManager.MotorRs[0].MotorIndex.ToString();
            }
            else if (m_BtnMotor[(int)EMotor.FLIP_R2] == selectedButton)
            {
                selectedButtonIndex = (int)EMotor.FLIP_R2;
                PnlContent.Visible = true;
                isMotorSelected = true;
                SetBatchMovePage(mBatchMovePageOutFlip);
                mUnit = CDefine.UNIT_ANGULAR;
                m_objMotor = mOutFlipManager.MotorRs[1].Axis;
                m_strSelectedMotor = mOutFlipManager.MotorRs[1].MotorIndex.ToString();
            }
            else if (m_BtnMotor[(int)EMotor.FLIP_Z] == selectedButton)
            {
                selectedButtonIndex = (int)EMotor.FLIP_Z;
                PnlContent.Visible = true;
                isMotorSelected = true;
                SetBatchMovePage(mBatchMovePageOutFlip);
        ...
        private void BtnOutConveyorAutoMode_Click(object sender, EventArgs e)
        {
            mOutFlipManager.IsOutConveyorAutoMode = true;
            // 버튼 로그 추가
            string strLog = string.Format("[{0}] [{1}]", "BtnOutConveyorAutoMode_Click", false);
            m_objDocument.SetUpdateButtonLog(this, strLog);
        }

        private void BtnOutConveyorManualMode_Click(object sender, EventArgs e)
        {
            mOutFlipManager.IsOutConveyorAutoMode = false;
            // 버튼 로그 추가
            string strLog = string.Format("[{0}] [{1}]", "BtnOutConveyorAutoMode_Click", false);
            m_objDocument.SetUpdateButtonLog(this, strLog);
        }
