...
namespace SVI_NFT_R
{
    public partial class CFormTeachInRobot : CFormCommon, CFormInterface
    {
        private readonly string ROBOT_NAME = "IN ROBOT";
        private readonly CDocument m_objDocument;
        private InRobot mInRobotManager;
        ...
        {
            new BatchMoveInformation(InRobot.EBatch.LoadGet),
            new BatchMoveInformation(InRobot.EBatch.LoadPut),
            new BatchMoveInformation(InRobot.EBatch.AlignZeroSend),
            new BatchMoveInformation(InRobot.EBatch.AlignSend),
            null,

            new BatchMoveInformation(InRobot.EBatch.AlignZeroSend),
            new BatchMoveInformation(InRobot.EBatch.AlignSend),
            new BatchMoveInformation(InRobot.EBatch.AlignCalibration),
            null,
            null,

            new BatchMoveInformation(InRobot.EBatch.Align_Verification_Process),
            new BatchMoveInformation(InRobot.EBatch.Robot_Verification_Process),
            new BatchMoveInformation(InRobot.EBatch.Align_Verification_Process2),
            new BatchMoveInformation(InRobot.EBatch.Robot_Verification_Process2),
            null,

        };
        private readonly BatchMoveInformation[] mBatchMovePageP3 = new BatchMoveInformation[]
        {
            null,
            new BatchMoveInformation(InRobot.EBatch.McrPut),
            null,
            null,
            null,
        };
        ...
        {
            new BatchMoveInformation(InRobot.EBatch.UnloadGet),
            new BatchMoveInformation(InRobot.EBatch.UnloadPut),
            null,
            null,
            null,
        };
        ...
        // MCR
        private readonly JcmCommandInformation[] mJcmCommandPageP3 = new JcmCommandInformation[]
        {
            new JcmCommandInformation(ERobotProcess.P3, EJcmCommand.T1),
            new JcmCommandInformation(ERobotProcess.P3, EJcmCommand.T2),
            new JcmCommandInformation(ERobotProcess.P3, EJcmCommand.T12),
        };
        // INSP STAGE
        ...
        private class BatchMoveInformation
        {
            ...
            /// 배치 동작 커맨드
            /// </summary>
            public InRobot.EBatch BatchCmd { get; private set; }

            public BatchMoveInformation(InRobot.EBatch batchCmd)
            {
                BatchCmd = batchCmd;
            }
        }
        ...
        public CFormTeachInRobot(CDocument objDocument)
        {
            // 도큐먼트 포인트 받음
        ...
        public bool Initialize()
        {
            ...
            do
            {
                mInRobotManager = m_objDocument.m_objProcessMain.m_objProcessMotion.InRobot;
        ...
        public bool InitializeForm()
        {
            ...
            do
            {
                PnlContentMiddleLeft.Controls.Add(PnlAlignDataLayout);
                PnlContentMiddleLeft.Controls.Add(PnlMcrDataLayout);
                PnlAlignDataLayout.Location = new Point(0, 0);
                PnlMcrDataLayout.Location = new Point(0, 0);
                ...
                UcTeachVacuum1.Initialize(this, m_objDocument, mInRobotManager.Pickers[0].VacuumIndex, CDeviceIODefine.EDigitalInput.X_IN_ROBOT_P1_CELL_DETECT_SENSOR, GetIsBlowAutoOff, GetIsAnalogDataView);
                UcTeachVacuum2.Initialize(this, m_objDocument, mInRobotManager.Pickers[1].VacuumIndex, CDeviceIODefine.EDigitalInput.X_IN_ROBOT_P2_CELL_DETECT_SENSOR, GetIsBlowAutoOff, GetIsAnalogDataView);
                mTeachCylinders = new UcTeachCylinder[]
                {
                    UcTeachCylinder1,
                    UcTeachCylinder2,
                };
                UcTeachCylinder1.Initialize(this, m_objDocument, mInRobotManager.TurnCylinders[0].CylinderIndex, mInRobotManager.TurnCylinders[0].m_objInterlock.CheckCylinderInterlock);
                UcTeachCylinder2.Initialize(this, m_objDocument, mInRobotManager.TurnCylinders[1].CylinderIndex, mInRobotManager.TurnCylinders[1].m_objInterlock.CheckCylinderInterlock);
        ...
        {
            BtnMcrDataP1.Tag = new BindingText(m_objDocument, BtnMcrDataP1, null, sender => mInRobotManager.Mcrs[0].GrabResult, null);
            BtnMcrDataP2.Tag = new BindingText(m_objDocument, BtnMcrDataP2, null, sender => mInRobotManager.Mcrs[1].GrabResult, null);
        ...
        private void SetButtonColor()
        {
            ...
            PnlOffsetDataLayout.BackColor = m_colorLabelCategory;
            SetButtonBackColor(BtnSubTitleOffsetDataP2, m_colorLabelData);
            SetButtonBackColor(BtnOffsetDataP2, m_colorLabelData);
            SetButtonBackColor(BtnSubTitleOffsetDataP1, m_colorLabelData);
            SetButtonBackColor(BtnOffsetDataP1, m_colorLabelData);

            PnlMcrDataLayout.BackColor = m_colorLabelCategory;
            SetButtonBackColor(BtnSubTitleMcrDataP2, m_colorLabelData);
            SetButtonBackColor(BtnMcrDataP2, m_colorLabelData);
            SetButtonBackColor(BtnSubTitleMcrDataP1, m_colorLabelData);
            SetButtonBackColor(BtnMcrDataP1, m_colorLabelData);
            ...
            foreach (SpeedButton btn in speedButtons)
            {
                if (
                    ...
                    && false == btn.Name.Equals(BtnMcrDataP2.Name)
                    && false == btn.Name.Equals(BtnMcrDataP1.Name)
        ...
        private void SetResourceControl()
        {
            ...
            SetButtonEnable(BtnMotorNachiP3, true);
        ...
        public bool SetChangeLanguage()
        {
            ...
            do
            {
                ...
                SetButtonChangeLanguage(BtnMotorNachiP3);
                ...
                SetLabelChangeLanguage(LblTitleOffsetData);
                SetLabelChangeLanguage(LblTitleMcrData);
        ...
        private void Timer_Tick(object sender, EventArgs e)
        {
            ...
            // 서보 기동 준비 신호
            // 티치 모드 전환 ( 교시 모드 )
            var robotStatus = mInRobotManager.Nachi.Robot.Status;
            ...
            // 일시 정지
            if (false == mInRobotManager.Nachi.Robot.Signals.Y.BB[Bit.EOutput.PAUSE].Value)
            {
            ...
            // 에러 메시지
            var errorCode = mInRobotManager.Nachi.Robot.Status.CurrentErrorCode;
            ...
            // 프로그램 선택 버튼 정보 표시
            {
                ...
                SetButtonBackColor(BtnMotorNachiP3, (mBatchMovePage == mBatchMovePageP3) ? m_colorClick : m_colorNormal);
                SetButtonBackColor(BtnMotorNachiP4, (mBatchMovePage == mBatchMovePageP4) ? m_colorClick : m_colorNormal);

                int currentPositionIndex = mInRobotManager.Nachi.Robot.Signals.X.WG[Word.EInputGroup.CURRENT_POS_BITS].Value;
                SetButtonBackColor(BtnCurrentPosition1, currentPositionIndex == 1 ? m_colorOn : m_colorNormal);
                SetButtonBackColor(BtnCurrentPosition3, currentPositionIndex == 3 ? m_colorOn : m_colorNormal);
                SetButtonBackColor(BtnCurrentPosition4, currentPositionIndex == 4 ? m_colorOn : m_colorNormal);
            }

            // 배치 동작 정보 표시
            {
                bool bIsJcmMode = mInRobotManager.Nachi.Robot.Status.IsJcmMode;
                ...
                for (int i = 0; i < BATCH_PAGE_ITME_MAX_COUNT; i++)
                {
                    ...
                    SetButtonBackColor(mBatchButtons[i], mInRobotManager.BatchCommand == mBatchMovePage[startIndex + i].BatchCmd ? m_colorClick : m_colorNormal);
            ...
            // JCM Mode
            {
                bool bJcmMode = mInRobotManager.Nachi.Robot.Status.IsJcmMode == true;
                ...
                bool bCanJcmCheckTeaching = (
                    bJcmMode == true
                    && mInRobotManager.Nachi.Robot.CanJcmEndCheckTeaching() == false
                    );
                ...
                for (int i = 0; i < JCM_COMMAND_PAGE_ITME_MAX_COUNT; i++)
                {
                    ...
                    bool bIsArrival = mInRobotManager.Nachi.Robot.Status.IsJcmCheckTeachingTool == mJcmCommandPage[startIndex + i].JcmCommand
                        && mInRobotManager.Nachi.Robot.Status.CurrentPosition == (int)mJcmCommandPage[startIndex + i].RobotProcess
                        ;
                ...
                bool bCanJcmEndCheckTeaching = mInRobotManager.Nachi.Robot.CanJcmEndCheckTeaching();
                ...
                // Align Data 업데이트
                {
                    ...
                    var alignData = inShuttle.AlignVision.AlignResults;
                    ...
                    SetButtonText(BtnOffsetDataP1, $"{"X",TITLE_MAX_LENGTH}: {mInRobotManager.Nachi.Robot.Status.Tool1AlignData_X,6:0.000} ( {CDefine.UNIT_MILLIMETER} )\n{"Y",TITLE_MAX_LENGTH}: {mInRobotManager.Nachi.Robot.Status.Tool1AlignData_Y,6:0.000} ( {CDefine.UNIT_MILLIMETER} )\n{"T",TITLE_MAX_LENGTH}: {mInRobotManager.Nachi.Robot.Status.Tool1AlignData_T,6:0.000} ( {CDefine.UNIT_ANGULAR} )");
                    SetButtonText(BtnOffsetDataP2, $"{"X",TITLE_MAX_LENGTH}: {mInRobotManager.Nachi.Robot.Status.Tool2AlignData_X,6:0.000} ( {CDefine.UNIT_MILLIMETER} )\n{"Y",TITLE_MAX_LENGTH}: {mInRobotManager.Nachi.Robot.Status.Tool2AlignData_Y,6:0.000} ( {CDefine.UNIT_MILLIMETER} )\n{"T",TITLE_MAX_LENGTH}: {mInRobotManager.Nachi.Robot.Status.Tool2AlignData_T,6:0.000} ( {CDefine.UNIT_ANGULAR} )");
                }
            }
        }
        ...
        {
            // 일시 정지
            if (true == mInRobotManager.Nachi.Robot.Signals.Y.BB[Bit.EOutput.PAUSE].Value)
            {
                mInRobotManager.Nachi.Robot.SetPause();
                // 버튼 로그 추가
            ...
            {
                mInRobotManager.Nachi.Robot.SetResume();
                // 버튼 로그 추가
        ...
        private async void BtnMotorInitialize_Click(object sender, EventArgs e)
        {
            do
            {
                ...
                if (false == mInRobotManager.Nachi.m_objInterlock.CheckMotionClassInterlock("", (int)InRobotNachi.ECommand.Initialize))
                {
                    break;
                }
                mInRobotManager.Nachi.SetCommand(InRobotNachi.ECommand.Initialize);
                bool bFunctionResult = await Task.Factory.StartNew(() => mInRobotManager.Nachi.WaitForEndProcess());
                if (false == bFunctionResult)
                {
                    break;
                }
                mInRobotManager.SetInitializeCommand(CProcessAbstract.EProcessManagerInitialize.PROCESS_MANAGER_INITIALIZE_DONE);
        ...
        private void BtnMotorNachiP1_Click(object sender, EventArgs e)
        {
            do
            {
                ...
                PnlAlignDataLayout.Visible = true;
                PnlMcrDataLayout.Visible = false;
                PnlContentMiddleLeft.Visible = true;
                PnlContentMiddleRight.Visible = true;
        ...
        private void BtnMotorNachiP3_Click(object sender, EventArgs e)
        {
            do
            {
                // 버튼 로그 추가
                string strLog = string.Format("[{0}] [{1}]", "BtnMotorNachiP3_Click", true);
                m_objDocument.SetUpdateButtonLog(this, strLog);
                mSelectProcess = ERobotProcess.P3;
                PnlAlignDataLayout.Visible = false;
                PnlMcrDataLayout.Visible = true;
                PnlContentMiddleLeft.Visible = true;
                PnlContentMiddleRight.Visible = false;

                SetJcmCommandPage(mJcmCommandPageP3);
                SetBatchMovePage(mBatchMovePageP3);

                // 버튼 로그 추가
                strLog = string.Format("[{0}] [{1}]", "BtnMotorNachiP3_Click", false);
                m_objDocument.SetUpdateButtonLog(this, strLog);
            } while (false);
        }
        ...
        private void BtnMotorNachiP4_Click(object sender, EventArgs e)
        {
            do
            {
                ...
                PnlAlignDataLayout.Visible = false;
                PnlMcrDataLayout.Visible = false;
                PnlContentMiddleLeft.Visible = false;
                PnlContentMiddleRight.Visible = false;
        ...
        private void ThreadRobotDryRunProcess()
        {
            ...
            while (false == m_bContinue)
            {
                ...
                switch (step)
                {
                    // Initialize
                    case 0x00:
                        if (mInRobotManager.Nachi.m_objInterlock.CheckMotionClassInterlock(string.Empty, (int)InRobotNachi.ECommand.Initialize) == false) break;
                        mInRobotManager.Nachi.SetCommand(InRobotNachi.ECommand.Initialize);
                        if (mInRobotManager.Nachi.WaitForEndProcess() == false) break;
                        bErrorOccured = false;
                        break;

                    // Loop
                    case 0x01:
                        ////if (mInRobotManager.Nachi.m_objInterlock.CheckMotionClassInterlock(string.Empty, (int)InRobotNachi.ECommand.LoadPermit) == false) break;
                        //mInRobotManager.Nachi.SetCommand(InRobotNachi.ECommand.LoadProcessBegin);
                        //if (mInRobotManager.Nachi.WaitForEndProcess() == false) break;
                        //mInRobotManager.Nachi.ProcessPreOrder.Method = EMethod.Get;
                        //mInRobotManager.Nachi.ProcessPreOrder.StageIndex = EStage.Stage2;
                        //mInRobotManager.Nachi.ProcessPreOrder.ToolIndex = ETool.Tool2;
                        //mInRobotManager.Nachi.ProcessMethodIndex = EMethod.Get;
                        //mInRobotManager.Nachi.ProcessStageIndex = EStage.Stage1;
                        //mInRobotManager.Nachi.ProcessToolIndex = ETool.Tool1;
                        //mInRobotManager.Nachi.SetCommand(InRobotNachi.ECommand.LoadPermit);
                        //if (mInRobotManager.Nachi.WaitForEndProcess() == false) break;
                        //mInRobotManager.Nachi.SetCommand(InRobotNachi.ECommand.LoadCycleEnd);
                        //if (mInRobotManager.Nachi.WaitForEndProcess() == false) break;
                        //mInRobotManager.Nachi.ProcessStageIndex = EStage.Stage2;
                        //mInRobotManager.Nachi.ProcessToolIndex = ETool.Tool2;
                        //mInRobotManager.Nachi.SetCommand(InRobotNachi.ECommand.LoadPermit);
                        //if (mInRobotManager.Nachi.WaitForEndProcess() == false) break;
                        //mInRobotManager.Nachi.SetCommand(InRobotNachi.ECommand.LoadCycleEnd);
                        //if (mInRobotManager.Nachi.WaitForEndProcess() == false) break;
                        //mInRobotManager.Nachi.SetCommand(InRobotNachi.ECommand.LoadProcessExit);
                        //if (mInRobotManager.Nachi.WaitForEndProcess() == false) break;
                        bErrorOccured = false;
                        break;

                    case 0x02:
                        ////if (mInRobotManager.Nachi.m_objInterlock.CheckMotionClassInterlock(string.Empty, (int)InRobotNachi.ECommand.UnloadPermit) == false) break;
                        //mInRobotManager.Nachi.SetCommand(InRobotNachi.ECommand.UnloadProcessBegin);
                        //if (mInRobotManager.Nachi.WaitForEndProcess() == false) break;
                        //mInRobotManager.Nachi.ProcessPreOrder.Method = EMethod.Put;
                        //mInRobotManager.Nachi.ProcessPreOrder.StageIndex = EStage.Stage2;
                        //mInRobotManager.Nachi.ProcessPreOrder.ToolIndex = ETool.Tool2;
                        //mInRobotManager.Nachi.ProcessMethodIndex = EMethod.Put;
                        //mInRobotManager.Nachi.ProcessStageIndex = EStage.Stage1;
                        //mInRobotManager.Nachi.ProcessToolIndex = ETool.Tool1;
                        //mInRobotManager.Nachi.SetCommand(InRobotNachi.ECommand.UnloadPermit);
                        //if (mInRobotManager.Nachi.WaitForEndProcess() == false) break;
                        //mInRobotManager.Nachi.SetCommand(InRobotNachi.ECommand.UnloadCycleEnd);
                        //if (mInRobotManager.Nachi.WaitForEndProcess() == false) break;
                        //mInRobotManager.Nachi.ProcessStageIndex = EStage.Stage2;
                        //mInRobotManager.Nachi.ProcessToolIndex = ETool.Tool2;
                        //mInRobotManager.Nachi.SetCommand(InRobotNachi.ECommand.UnloadPermit);
                        ...
                        //mInRobotManager.Nachi.SetCommand(InRobotNachi.ECommand.UnloadCycleEnd);
                        //if (mInRobotManager.Nachi.WaitForEndProcess() == false) break;
                        //mInRobotManager.Nachi.SetCommand(InRobotNachi.ECommand.UnloadProcessExit);
                        //if (mInRobotManager.Nachi.WaitForEndProcess() == false) break;
                        bErrorOccured = false;
                        break;
        ...
        private void SetBatchMoveExecuteItem(int index)
        {
            do
            {
                ...
                if (InRobot.EBatch.None != mInRobotManager.BatchCommand)
                {
                    break;
                }
                ...
                // Batch Move Execute
                mInRobotManager.BatchCommand = mBatchMovePage[targetIndex].BatchCmd;

                // Batch Move End Wait
                while (InRobot.EBatch.None != mInRobotManager.BatchCommand)
                {
                    Application.DoEvents();
                    Thread.Sleep(10);
                }
        ...
        {
            if (
                mInRobotManager.Nachi.Robot.CanJcmExitMode() == false
                || mInRobotManager.Nachi.Robot.CanJcmEndCheckTeaching() == true
                )
            {
                return;
            }
            ...
            if (mInRobotManager.BatchCommand != InRobot.EBatch.None)
            {
                return;
            }
            ...
            try
            {
                if (mInRobotManager.Nachi.Robot.JcmSelectProcess(mSelectProcess) == false)
                {
                    return;
                }
                ...
                if (mInRobotManager.Nachi.Robot.SendAlignData(alignData) == false)
                {
                    return;
                }
                if (mInRobotManager.Nachi.Robot.JcmCheckTeaching(mJcmCommandPage[targetIndex].JcmCommand) == false)
                {
                    return;
                }
            }
        ...
        private async void BtnJcmMode_Click(object sender, EventArgs e)
        {
            ...
            do
            {
                if (mInRobotManager.Nachi.Robot.Status.IsJcmMode == true)
                {
                    // 버튼 로그 추가
                    ...
                    // JCM 모드 리셋
                    {
                        ...
                        if (await Task.Factory.StartNew(() => mInRobotManager.Nachi.Robot.JcmExitMode()) == false)
                        {
                ...
                {
                    if (mInRobotManager.Nachi.Robot.CanJcmEnterMode() == false)
                    {
                    ...
                    // JCM 모드 설정
                    {
                        ...
                        if (await Task.Factory.StartNew(() => mInRobotManager.Nachi.Robot.JcmEnterMode()) == false)
                        {
        ...
        {
            if (mInRobotManager.Nachi.Robot.CanJcmEndCheckTeaching() == false)
            {
                return;
            }
            if (m_objDocument.m_objProcessMain.CheckDoorLock() == false)
            ...
            try
            {
                if (await Task.Factory.StartNew(() => mInRobotManager.Nachi.Robot.JcmCheckTeachingSave()) == false)
                {
                    return;
                }
            }
        ...
        private async void BtnJcmModeNoSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (await Task.Factory.StartNew(() => mInRobotManager.Nachi.Robot.JcmCheckTeachingNoSave()) == false)
                {
                    return;
                }
            }
        ...
        private async void BtnMcrDataP1_Click(object sender, EventArgs e)
        {
            if (m_objDocument.m_objProcessMain.m_objProcessMotion.CheckAllSequenceManagerStateIsBatchMoving() == true)
            {
                return;
            ...
            m_objDocument.GetMainFrame().ShowWaitMessage(true, $"MCR GRAB P1");
            mInRobotManager.BatchCommand = InRobot.EBatch.McrGrabP1;
            await Task.Run(() =>
            {
                while (mInRobotManager.BatchCommand != InRobot.EBatch.None)
                {
                    Thread.Sleep(10);
                }
            });
            m_objDocument.GetMainFrame().ShowWaitMessage(false);
            m_objDocument.SetUpdateButtonLog(this, $"[{MethodBase.GetCurrentMethod().Name}] [MCR Trigger P1]");
        }

        private async void BtnMcrDataP2_Click(object sender, EventArgs e)
        {
            if (m_objDocument.m_objProcessMain.m_objProcessMotion.CheckAllSequenceManagerStateIsBatchMoving() == true)
            {
                return;
            ...
            m_objDocument.GetMainFrame().ShowWaitMessage(true, $"MCR GRAB P2");
            mInRobotManager.BatchCommand = InRobot.EBatch.McrGrabP2;
            await Task.Run(() =>
            {
                while (mInRobotManager.BatchCommand != InRobot.EBatch.None)
                {
        ...
        private async void BtnAlignDataP1_Click(object sender, EventArgs e)
        {
            if (m_objDocument.m_objProcessMain.m_objProcessMotion.CheckAllSequenceManagerStateIsBatchMoving() == true)
            {
                return;
            ...
            m_objDocument.GetMainFrame().ShowWaitMessage(true, $"ALIGN GRAB P1");
            var inShuttle = m_objDocument.m_objProcessMain.m_objProcessMotion.InShuttle;
            inShuttle.BatchCommand = InShuttle.EBatch.AlignGrabP1;
            await Task.Run(() =>
            {
                while (inShuttle.BatchCommand != InShuttle.EBatch.None)
                {
                    Thread.Sleep(10);
                }
            });
            m_objDocument.GetMainFrame().ShowWaitMessage(false);
            m_objDocument.SetUpdateButtonLog(this, $"[{MethodBase.GetCurrentMethod().Name}] [ALIGN Trigger P1]");
        }

        private async void BtnAlignDataP2_Click(object sender, EventArgs e)
        {
            if (m_objDocument.m_objProcessMain.m_objProcessMotion.CheckAllSequenceManagerStateIsBatchMoving() == true)
            {
                return;
            ...
            m_objDocument.GetMainFrame().ShowWaitMessage(true, $"ALIGN GRAB P2");
            var inShuttle = m_objDocument.m_objProcessMain.m_objProcessMotion.InShuttle;
            inShuttle.BatchCommand = InShuttle.EBatch.AlignGrabP2;
            await Task.Run(() =>
            {
                while (inShuttle.BatchCommand != InShuttle.EBatch.None)
                {
            ...
            m_objDocument.GetMainFrame().ShowWaitMessage(false);
            m_objDocument.SetUpdateButtonLog(this, $"[{MethodBase.GetCurrentMethod().Name}] [ALIGN Trigger P2]");
        }
    }
}