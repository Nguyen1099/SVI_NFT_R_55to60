...
namespace SVI_NFT_R
{
    public partial class CFormTeachOutShuttle : CFormCommon, CFormInterface
    {
        private enum EMotor
        {
            STAGE_X = 0,
        ...
        private OutShuttle mOutShuttleManager;
        ...
        {
            new BatchMoveInformation(OutShuttle.EBatch.MoveLoadPosition),
            new BatchMoveInformation(OutShuttle.EBatch.MoveUnloadPosition),
            null,
            null,
            null,
        ...
        private class BatchMoveInformation
        {
            ...
            /// 배치 동작 커맨드
            /// </summary>
            public OutShuttle.EBatch BatchCmd { get; private set; }

            public BatchMoveInformation(OutShuttle.EBatch batchCmd)
            {
                BatchCmd = batchCmd;
            }
        }
        ...
        /// <returns></returns>
        public CFormTeachOutShuttle(CDocument objDocument)
        {
            // 도큐먼트 포인트 받음
        ...
        public bool Initialize()
        {
            ...
            do
            {
                mOutShuttleManager = m_objDocument.m_objProcessMain.m_objProcessMotion.OutShuttle;
        ...
        public bool InitializeForm()
        {
            ...
            do
            {
                ...
                // 초기 모터 객체 설정
                m_objMotor = mOutShuttleManager.MotorStageX.Axis;
                mInterlock = mOutShuttleManager.MotorStageX.m_objInterlock;
                ...
                UcTeachVacuum1.Initialize(this, m_objDocument, mOutShuttleManager.Pickers[0].VacuumIndex, CDeviceIODefine.EDigitalInput.X_IN_SHUTTLE_P1_CELL_DETECT_SENSOR, GetIsBlowAutoOff, GetIsAnalogDataView);
                UcTeachVacuum2.Initialize(this, m_objDocument, mOutShuttleManager.Pickers[1].VacuumIndex, CDeviceIODefine.EDigitalInput.X_IN_SHUTTLE_P2_CELL_DETECT_SENSOR, GetIsBlowAutoOff, GetIsAnalogDataView);
                // 배치 이동 페이지 선택
        ...
        private void Timer_Tick(object sender, EventArgs e)
        {
            ...
            // 배치 동작 정보 표시
            {
                ...
                for (int i = 0; i < BATCH_PAGE_ITME_MAX_COUNT; i++)
                {
                    ...
                    if (mOutShuttleManager.BatchCommand == mBatchMovePage[startIndex + i].BatchCmd)
                    {
        ...
        private void SetBatchMoveExecuteItem(int index)
        {
            do
            {
                ...
                // Batch Move Execute
                mOutShuttleManager.BatchCommand = mBatchMovePage[targetIndex].BatchCmd;

                // Batch Move End Wait
                while (OutShuttle.EBatch.None != mOutShuttleManager.BatchCommand)
                {
        ...
        private void ShowMotorVisible(ImageButton selectedButton, out int selectedButtonIndex)
        {
            ...
            if (m_BtnMotor[(int)EMotor.STAGE_X] == selectedButton)
            {
                ...
                m_objMotor = mOutShuttleManager.MotorStageX.Axis;
                m_strSelectedMotor = mOutShuttleManager.MotorStageX.MotorIndex.ToString();
            }
