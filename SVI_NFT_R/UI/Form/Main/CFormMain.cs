...
namespace SVI_NFT_R
{
    public partial class CFormMain : CFormCommon, CFormInterface
    {
        ...
        /// 현재 폼
        /// </summary>
        private CDefine.EFormViewMain m_eCurrentForm = CDefine.EFormViewMain.FORM_VIEW_MAIN_FINAL;
        /// <summary>
        /// 폼 (화면이 전환되는 폼)
        /// </summary>
        ...
        public bool InitializeForm()
        {
            ...
            do
            {
                int iMenuCount = (int)CDefine.EFormViewMain.FORM_VIEW_MAIN_FINAL;
                // 버튼 여백 & 넓이 설정
                int iWhiteSpace = 2;
                ...
                {
                    strButtonName[iLoopButton] = ((CDefine.EFormViewMain)iLoopButton).ToString();
                }
                // 버튼 동적 생성
                ...
                // 화면 전환
                SetChangeForm(CDefine.EFormViewMain.FORM_VIEW_MAIN_FLOW);

                // 타이머 설정
                timer.Interval = 100;
                timer.Enabled = true;

                timerMCRDlg.Interval = 100;
                timerMCRDlg.Enabled = true;
        ...
        public void SetChangeForm(CDefine.EFormViewMain eForm)
        {
            do
            {
                // 현재 폼이 이전 폼과 같으면 건너뜀
                ...
                // 현재 폼 Invisible
                if (CDefine.EFormViewMain.FORM_VIEW_MAIN_FINAL != m_eCurrentForm)
                {
                ...
                if (null == m_stForm[(int)eForm].m_IForm)
                {
                    switch (eForm)
                    {
                        case CDefine.EFormViewMain.FORM_VIEW_MAIN_FLOW:
                            m_stForm[(int)eForm] = new structureForm(new CFormMainFlow(m_objDocument) as CFormInterface);
                            break;
                        case CDefine.EFormViewMain.FORM_VIEW_MAIN_CIM:
                            m_stForm[(int)eForm] = new structureForm(new CFormMainCIM(m_objDocument) as CFormInterface);
                            break;
                        default:
                            break;
                    }
        ...
        public bool SetChangeLanguage()
        {
            ...
            do
            {
                ...
                SetButtonChangeLanguage(m_btnMenu[(int)CDefine.EFormViewMain.FORM_VIEW_MAIN_FLOW]);
                SetButtonChangeLanguage(m_btnMenu[(int)CDefine.EFormViewMain.FORM_VIEW_MAIN_CIM]);
        ...
        public void ButtonMenu_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (true == objRegex.IsMatch(objButton.Name))
                {
                    ...
                    CDefine.EFormViewMain eFormView = (CDefine.EFormViewMain)Convert.ToInt32(strText);
        ...
        {
            for (int iLoopMenu = 0; iLoopMenu < (int)CDefine.EFormViewMain.FORM_VIEW_MAIN_FINAL; iLoopMenu++)
            {
        ...
        private void timerMCRDlg_Tick(object sender, EventArgs e)
        {
            ////Y_Y11C X_X004
            //if (m_objDocument.GetIOBit(CDeviceIODefine.DeviceInput.X_CELL_A_MCR_READ_STAND_BY) == true)
            //{
            //    if (m_objDocument.m_dialogMCRPopupA.Visible == true)
            //        m_objDocument.m_dialogMCRPopupA.Hide();       
            //    //MCR POPup 발생
            //    if (m_objDocument.m_dialogMCRPopupA.Visible == false)
            //    {
            //        m_objDocument.SetIOBit(CDeviceIODefine.DeviceOutput.Y_CELL_A_MCR_READ_FINISH_LAMP, CDefine.OFF);
            //        m_objDocument.m_dialogMCRPopupA.m_ePos = CDefine.enumPosition.POSITION_A;
            //        m_objDocument.m_dialogMCRPopupA.Show();
            //    }
            //}
            //else if (m_objDocument.GetIOBit(CDeviceIODefine.DeviceInput.X_CELL_B_MCR_READ_STAND_BY) == true)
            //{
            //    if (m_objDocument.m_dialogMCRPopupB.Visible == true)
            //        m_objDocument.m_dialogMCRPopupB.Hide();
            //    //MCR POPup 발생
            //    if (m_objDocument.m_dialogMCRPopupB.Visible == false)
            //    {
            //        m_objDocument.SetIOBit(CDeviceIODefine.DeviceOutput.Y_CELL_B_MCR_READ_FINISH_LAMP, CDefine.OFF);
            //        m_objDocument.m_dialogMCRPopupB.m_ePos = CDefine.enumPosition.POSITION_B;
            //        m_objDocument.m_dialogMCRPopupB.Show();
            //    }
            //}
        }
    }
