...
namespace SVI_NFT_R
{
    public partial class CFormStatisticsDBEvent : CFormCommon, CFormInterface
    {
        /// <summary>
        /// 도큐먼트
        /// </summary>
        ...
        /// 현재 선택 메뉴
        /// </summary>
        private CDefine.EStatisticsDBEvent m_eCurrentStatisticsDB = CDefine.EStatisticsDBEvent.STATISTICS_DB_EVENT_ALARM;
        /// <summary>
        /// 현재 매니저 테이블
        /// </summary>
        ...
        /// <returns></returns>
        public CFormStatisticsDBEvent(CDocument objDocument)
        {
            // 도큐먼트 포인트 받음
        ...
        /// <param name="e"></param>
        private void CFormStatisticsDBEvent_Load(object sender, EventArgs e)
        {
            // 초기화
            Initialize();
        }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void CFormStatisticsDBEvent_FormClosed(object sender, FormClosedEventArgs e)
        {
            // 해제
            DeInitialize();
        }
        ...
        public bool InitializeForm()
        {
            ...
            do
            {
                ...
                int iMenuCount = (int)CDefine.EStatisticsDBEvent.STATISTICS_DB_EVENT_FINAL;
                // 버튼 여백 & 넓이 설정
                int iWhiteSpace = 2;
                ...
                {
                    strButtonName[iLoopButton] = ((CDefine.EStatisticsDBEvent)iLoopButton).ToString();
                }
                // 버튼 동적 생성
                ...
                {
                    m_btnMenu[iLoopMenu].Name = string.Format("BtnStatisticsDBEvent[{0}]", iLoopMenu);
                }
                ...
                // ! 설비에서 Safety PLC 통신을 사용하지 않는 경우 버튼을 숨김
                m_btnMenu[(int)CDefine.EStatisticsDBEvent.STATISTICS_DB_EVENT_SAFETY_PLC_MODIFIED_TIME].Visible = Enum.GetValues(typeof(CDefine.ESafetyPlc)).Length != 0;
        ...
        private new bool InitializeGridView(DataGridView objGridView)
        {
            ...
            do
            {
                ...
                {
                    case CDefine.EStatisticsDBEvent.STATISTICS_DB_EVENT_ALARM:
                        objGridView.Columns[(int)CDatabaseDefine.EHistoryAlarmEvent.DATE].Width = 150;
                        objGridView.Columns[(int)CDatabaseDefine.EHistoryAlarmEvent.EVENT].Width = 100;
                        break;
                    case CDefine.EStatisticsDBEvent.STATISTICS_DB_EVENT_EQUIPMENT_STOP:
                        objGridView.Columns[(int)CDatabaseDefine.EHistoryEquipmentStopEvent.DATE].Width = 150;
                        objGridView.Columns[(int)CDatabaseDefine.EHistoryEquipmentStopEvent.EVENT].Width = 100;
                        break;
                    case CDefine.EStatisticsDBEvent.STATISTICS_DB_EVENT_EQUIPMENT_TP_CODE:
                        objGridView.Columns[(int)CDatabaseDefine.EHistoryEquipmentTPCodeEvent.DATE].Width = 150;
                        objGridView.Columns[(int)CDatabaseDefine.EHistoryEquipmentTPCodeEvent.TP_CODE].Width = 200;
                        break;
                    case CDefine.EStatisticsDBEvent.STATISTICS_DB_EVENT_EQUIPMENT_UPPER_LOSS_TIME:
                        objGridView.Columns[(int)CDatabaseDefine.EHistoryEquipmentUpperLossTimeEvent.DATE].Width = 150;
                        objGridView.Columns[(int)CDatabaseDefine.EHistoryEquipmentUpperLossTimeEvent.EVENT].Width = 100;
                        break;
                    case CDefine.EStatisticsDBEvent.STATISTICS_DB_EVENT_EQUIPMENT_LOWER_LOSS_TIME:
                        objGridView.Columns[(int)CDatabaseDefine.EHistoryEquipmentLowerLossTimeEvent.DATE].Width = 150;
                        objGridView.Columns[(int)CDatabaseDefine.EHistoryEquipmentLowerLossTimeEvent.EVENT].Width = 100;
                        break;
                    case CDefine.EStatisticsDBEvent.STATISTICS_DB_EVENT_SAFETY_PLC_MODIFIED_TIME:
                        objGridView.Columns[(int)CDatabaseDefine.EHistorySafetyPlcModifiedEvent.LAST_MODIFIED_TIME].Width = 150;
                        objGridView.Columns[(int)CDatabaseDefine.EHistorySafetyPlcModifiedEvent.ID].Width = 150;
                        objGridView.Columns[(int)CDatabaseDefine.EHistorySafetyPlcModifiedEvent.SIGNATURE_CODE].Width = 150;
                        break;
                    default:
                        break;
                }
        ...
        private void SetDatabaseConnect(CDefine.EStatisticsDBEvent eStatisticsDB, DateTime objFrom, DateTime objTo)
        {
            // 디폴트 내림차순
        ...
        private void SetDatabaseConnect(CDefine.EStatisticsDBEvent eStatisticsDB, DateTime objFrom, DateTime objTo, string strOrder)
        {
            ...
            {
                case CDefine.EStatisticsDBEvent.STATISTICS_DB_EVENT_ALARM:
                    m_objManagerTable = m_objDocument.m_objProcessDatabase.m_objProcessDatabaseHistory.m_objManagerTableHistoryAlarmEvent;
                    iIndexDate = (int)CDatabaseDefine.EHistoryAlarmEvent.DATE;
                    break;
                case CDefine.EStatisticsDBEvent.STATISTICS_DB_EVENT_EQUIPMENT_STOP:
                    m_objManagerTable = m_objDocument.m_objProcessDatabase.m_objProcessDatabaseHistory.m_objManagerTableHistoryEquipmentStopEvent;
                    iIndexDate = (int)CDatabaseDefine.EHistoryEquipmentStopEvent.DATE;
                    break;
                case CDefine.EStatisticsDBEvent.STATISTICS_DB_EVENT_EQUIPMENT_TP_CODE:
                    m_objManagerTable = m_objDocument.m_objProcessDatabase.m_objProcessDatabaseHistory.m_objManagerTableHistoryEquipmentTPCodeEvent;
                    iIndexDate = (int)CDatabaseDefine.EHistoryEquipmentTPCodeEvent.DATE;
                    break;
                case CDefine.EStatisticsDBEvent.STATISTICS_DB_EVENT_EQUIPMENT_UPPER_LOSS_TIME:
                    m_objManagerTable = m_objDocument.m_objProcessDatabase.m_objProcessDatabaseHistory.m_objManagerTableHistoryEquipmentUpperLossTimeEvent;
                    iIndexDate = (int)CDatabaseDefine.EHistoryEquipmentUpperLossTimeEvent.DATE;
                    break;
                case CDefine.EStatisticsDBEvent.STATISTICS_DB_EVENT_EQUIPMENT_LOWER_LOSS_TIME:
                    m_objManagerTable = m_objDocument.m_objProcessDatabase.m_objProcessDatabaseHistory.m_objManagerTableHistoryEquipmentLowerLossTimeEvent;
                    iIndexDate = (int)CDatabaseDefine.EHistoryEquipmentLowerLossTimeEvent.DATE;
                    break;
                case CDefine.EStatisticsDBEvent.STATISTICS_DB_EVENT_SAFETY_PLC_MODIFIED_TIME:
                    m_objManagerTable = m_objDocument.m_objProcessDatabase.m_objProcessDatabaseHistory.m_objManagerTableHistorySafetyPlcModifiedEvent;
                    iIndexDate = (int)CDatabaseDefine.EHistorySafetyPlcModifiedEvent.LAST_MODIFIED_TIME;
                    break;
                default:
                    break;
            }
        ...
        public void ButtonMenu_Click(object sender, EventArgs e)
        {
            ...
            CDefine.EStatisticsDBEvent eStatisticsDB = 0;
            try
            {
                ...
                if (true == objRegex.IsMatch(objButton.Name))
                {
                    ...
                    eStatisticsDB = (CDefine.EStatisticsDBEvent)Convert.ToInt32(strText);
            ...
            // 날짜 선택 컨트롤을 현재 날짜로 설정
            if (eStatisticsDB != CDefine.EStatisticsDBEvent.STATISTICS_DB_EVENT_SAFETY_PLC_MODIFIED_TIME)
            {
                this.DateTimeFrom.Value = DateTime.Now;
            }
            else
            {
                // !!! 세이프티PLC에 과거 수정 이력을 한 번에 보여줘야 하기 때문에 예외 처리함
                this.DateTimeFrom.Value = this.DateTimeFrom.MinDate;
            }
        ...
        {
            for (int iLoopMenu = 0; iLoopMenu < (int)CDefine.EStatisticsDBEvent.STATISTICS_DB_EVENT_FINAL; iLoopMenu++)
            {
