...
namespace SVI_NFT_R
{
    public partial class CFormConfig : CFormCommon, CFormInterface
    {
        ...
        /// 현재 폼
        /// </summary>
        private CDefine.EFormViewConfig m_eCurrentForm = CDefine.EFormViewConfig.FORM_VIEW_CONFIG_FINAL;
        /// <summary>
        /// 폼 (화면이 전환되는 폼)
        /// </summary>
        ...
        public bool InitializeForm()
        {
            ...
            do
            {
                int iMenuCount = (int)CDefine.EFormViewConfig.FORM_VIEW_CONFIG_FINAL;
                // 버튼 여백 & 넓이 설정
                int iWhiteSpace = 2;
                ...
                {
                    strButtonName[iLoopButton] = ((CDefine.EFormViewConfig)iLoopButton).ToString();
                }
                // 버튼 동적 생성
        ...
        public void SetChangeForm(CDefine.EFormViewConfig eForm)
        {
            do
            {
                // 현재 폼이 이전 폼과 같으면 건너뜀
                if (m_eCurrentForm == eForm)
                    break;
                // 현재 폼 Invisible
                if (CDefine.EFormViewConfig.FORM_VIEW_CONFIG_FINAL != m_eCurrentForm)
                {
                ...
                if (null == m_stForm[(int)eForm].m_IForm)
                {
                    switch (eForm)
                    {
                        case CDefine.EFormViewConfig.FORM_VIEW_CONFIG_OPTION:
                            m_stForm[(int)eForm] = new structureForm(new CFormConfigOption(m_objDocument) as CFormInterface);
                            break;
                        case CDefine.EFormViewConfig.FORM_VIEW_CONFIG_RECIPE:
                            m_stForm[(int)eForm] = new structureForm(new CFormConfigRecipe(m_objDocument) as CFormInterface);
                            break;
                        case CDefine.EFormViewConfig.FORM_VIEW_CONFIG_OFFSET:
                            m_stForm[(int)eForm] = new structureForm(new CFormConfigOffset(m_objDocument) as CFormInterface);
                            break;
                        case CDefine.EFormViewConfig.FORM_VIEW_CONFIG_WAIT_TIME_SETTING:
                            m_stForm[(int)eForm] = new structureForm(new CFormConfigWaitTime(m_objDocument) as CFormInterface);
                            break;
                        case CDefine.EFormViewConfig.FORM_VIEW_CONFIG_SIGNAL_TOWER:
                            m_stForm[(int)eForm] = new structureForm(new CFormConfigSignalTower(m_objDocument) as CFormInterface);
                            break;
                        case CDefine.EFormViewConfig.FORM_VIEW_CONFIG_MCC_LOG:
                            m_stForm[(int)eForm] = new structureForm(new CFormConfigMCCLog(m_objDocument) as CFormInterface);
                            break;
                        default:
                            break;
                    }
        ...
        public bool SetChangeLanguage()
        {
            ...
            do
            {
                ...
                SetButtonChangeLanguage(m_btnMenu[(int)CDefine.EFormViewConfig.FORM_VIEW_CONFIG_OPTION]);
                SetButtonChangeLanguage(m_btnMenu[(int)CDefine.EFormViewConfig.FORM_VIEW_CONFIG_RECIPE]);
                SetButtonChangeLanguage(m_btnMenu[(int)CDefine.EFormViewConfig.FORM_VIEW_CONFIG_OFFSET]);
                SetButtonChangeLanguage(m_btnMenu[(int)CDefine.EFormViewConfig.FORM_VIEW_CONFIG_WAIT_TIME_SETTING]);
                SetButtonChangeLanguage(m_btnMenu[(int)CDefine.EFormViewConfig.FORM_VIEW_CONFIG_SIGNAL_TOWER]);
                SetButtonChangeLanguage(m_btnMenu[(int)CDefine.EFormViewConfig.FORM_VIEW_CONFIG_MCC_LOG]);
        ...
        public void ButtonMenu_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (true == objRegex.IsMatch(objButton.Name))
                {
                    ...
                    CDefine.EFormViewConfig eFormView = (CDefine.EFormViewConfig)Convert.ToInt32(strText);
        ...
        {
            for (int iLoopMenu = 0; iLoopMenu < (int)CDefine.EFormViewConfig.FORM_VIEW_CONFIG_FINAL; iLoopMenu++)
            {
