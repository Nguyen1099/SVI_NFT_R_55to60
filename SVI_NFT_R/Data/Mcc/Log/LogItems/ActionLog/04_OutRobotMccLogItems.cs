...
namespace Mcc
{
    public sealed class OutRobotMccLogItems
    {
        ...
        public IMccLogItem[] INSP_RESULT_WAIT_TIME { get; set; } = new IMccLogItem[2];
        ...
        {
            // 인스턴스 생성
            var loadProcess = EProcess.InspStage;
            var process = EProcess.OutRobot;
            var Modules = new[]
            {
                UnitName.OutRobot01,
                UnitName.OutRobot02
            ...
            var LD = new[]
            {
                new { From = UnitName.InspectionStage01, To = Modules[0] },
                new { From = UnitName.InspectionStage02, To = Modules[1] }
            };
            var Proc = new[]
            {
            ...
            var ULD = new[]
            {
                new { From = Modules[0], To = UnitName.OutFlip01 },
                new { From = Modules[1], To = UnitName.OutFlip02 }
            };
            ...
            for (int p = 0; p < 2; ++p)
            {
                ...
                INSP_RESULT_WAIT_TIME[p] = new MccActionLogItem(document, process, p, $"{MccLogManager.GetSystemNo(systemNoPrefixs[p], systemNo++)}_", $"INSP_RESULT_WAIT_TIME", Modules[p], Proc[p].From, Proc[p].To);
