...
namespace Mcc
{
    public sealed class InShuttleMccLogItems
    {
        ...
        public IMccLogItem[] MOVE_X_LD_TO_INSP_START_POS { get; private set; } = new IMccLogItem[2];
        public IMccLogItem[] MOVE_X_INSP_START_TO_ALIGN_POS { get; private set; } = new IMccLogItem[2];
        public IMccLogItem[] VISION_ALIGN_PROCESS { get; private set; } = new IMccLogItem[2];
        public IMccLogItem[] MOVE_X_ALIGN_TO_ULD_POS { get; private set; } = new IMccLogItem[2];
        ...
        {
            // 인스턴스 생성
            var process = EProcess.InShuttle;
            var Modules = new[]
                {
                    UnitName.InShuttle01,
                    UnitName.InShuttle02
                };
            var LD = new[]
                {
            ...
            var ULD = new[]
                {
                    new { From = Modules[0], To = UnitName.InRobot01 },
                    new { From = Modules[1], To = UnitName.InRobot02 },
                };
            ...
            for (int p = 0; p < 2; p++)
            {
                ...
                MOVE_X_LD_TO_INSP_START_POS[p] = new MccActionLogItem(document, process, p, $"{MccLogManager.GetSystemNo(systemNoPrefixs[p], systemNo++)}_", $"MOVE_X_LD_TO_INSP_START_POS", Modules[p], Proc[p].From, Proc[p].To);
                MOVE_X_INSP_START_TO_ALIGN_POS[p] = new MccActionLogItem(document, process, p, $"{MccLogManager.GetSystemNo(systemNoPrefixs[p], systemNo++)}_", $"MOVE_X_INSP_START_TO_ALIGN_POS", Modules[p], Proc[p].From, Proc[p].To);

                VISION_ALIGN_PROCESS[p] = new MccActionLogItem(document, process, p, $"{MccLogManager.GetSystemNo(systemNoPrefixs[p], systemNo++)}_", $"VISION_ALIGN_PROCESS", Modules[p], Proc[p].From, Proc[p].To);
                MOVE_X_ALIGN_TO_ULD_POS[p] = new MccActionLogItem(document, process, p, $"{MccLogManager.GetSystemNo(systemNoPrefixs[p], systemNo++)}_", $"MOVE_X_ALIGN_TO_ULD_POS", Modules[p], Proc[p].From, Proc[p].To);
