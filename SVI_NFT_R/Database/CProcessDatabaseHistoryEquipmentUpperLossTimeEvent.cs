...
namespace SVI_NFT_R
{
    class CProcessDatabaseHistoryEquipmentUpperLossTimeEvent : CDatabaseAbstract
    {
        /// <summary>
        /// 이전 삽입 날짜
        /// </summary>
        ...
        public bool Initialize(CProcessDatabase objProcessDatabase)
        {
            ...
            do
            {
                ...
                // 딜레이 시간 설정 (ms)
                m_iDelayTime = Config.WaitTime.Equipment.LoadInterfaceDelayTime.ToMilliseconds();
                // 데이터베이스에서 초기에 조회해서 SET, CLEAR 상태 확인
                DataTable objDataTable = new DataTable();
                string query = string.Format("SELECT * FROM {0} ORDER BY {1} DESC LIMIT 1;",
                    m_objProcessDatabase.m_objProcessDatabaseHistory.m_objManagerTableHistoryEquipmentUpperLossTimeEvent.HLGetTableName(),
                    m_objProcessDatabase.m_objProcessDatabaseHistory.m_objManagerTableHistoryEquipmentUpperLossTimeEvent.HLGetTableSchemaName()[(int)CDatabaseDefine.EHistoryEquipmentUpperLossTimeEvent.DATE]
                    );
                ...
                try
                {
                    ...
                    else if (CDatabaseDefine.EEventValue.SET.ToString() == objDataRow[objDataRow.Length - 1].ItemArray[(int)CDatabaseDefine.EHistoryEquipmentUpperLossTimeEvent.EVENT].ToString())
                    {
                        m_objPreviousDate = DateTime.ParseExact(objDataRow[objDataRow.Length - 1].ItemArray[(int)CDatabaseDefine.EHistoryEquipmentUpperLossTimeEvent.DATE].ToString(), CDatabaseDefine.DEF_DATE_TIME_FORMAT, CultureInfo.InvariantCulture);
                        m_eEventValue = CDatabaseDefine.EEventValue.SET;
                    }
                    else if (CDatabaseDefine.EEventValue.CLEAR.ToString() == objDataRow[objDataRow.Length - 1].ItemArray[(int)CDatabaseDefine.EHistoryEquipmentUpperLossTimeEvent.EVENT].ToString())
                    {
                        m_objPreviousDate = DateTime.ParseExact(objDataRow[objDataRow.Length - 1].ItemArray[(int)CDatabaseDefine.EHistoryEquipmentUpperLossTimeEvent.DATE].ToString(), CDatabaseDefine.DEF_DATE_TIME_FORMAT, CultureInfo.InvariantCulture);
                        m_eEventValue = CDatabaseDefine.EEventValue.CLEAR;
                    }
        ...
        {
            CProcessDatabaseHistoryEquipmentUpperLossTimeEvent pThis = (CProcessDatabaseHistoryEquipmentUpperLossTimeEvent)state;

            while (false == pThis.m_bThreadExit)
            {
                // 앞 설비 지연 시간 이벤트 삽입
                pThis.SetInsertUpperLossTimeEvent();
        ...
        /// <summary>
        /// 상류 설비와 인터페이스 중 지연 시간 이벤트를 체크
        /// </summary>
        private void SetInsertUpperLossTimeEvent()
        {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        if (CCIMDefine.EMoveState.MOVE_STATE_RUNNING == m_objProcessDatabase.GetDocument().m_eMoveState[(int)CCIMDefine.EPresentState.CURRENT_STATE])
                        {
                            ...
                            if (true == m_objProcessDatabase.GetDocument().m_objProcessMain.m_objProcessMotion.LoadInterface.IsPending)
                            {
                                // 바로 이전 이벤트 CLEAR인 경우
                                if (CDatabaseDefine.EEventValue.CLEAR == m_eEventValue)
                                {
                                    ...
                                    {
                                        // Insert Set
                                        m_objProcessDatabase.m_objDatabaseSendMessage.SetInsertHistoryEquipmentUpperLossTimeEvent(CDatabaseDefine.EEventValue.SET);
                                        m_eEventValue = CDatabaseDefine.EEventValue.SET;
                                        m_objPreviousDate = DateTime.Now;
                                        // 딜레이 시간 초기화
                                        m_iDelayTime = Config.WaitTime.Equipment.LoadInterfaceDelayTime.ToMilliseconds();
                            ...
                            else
                            {
                                ...
                                {
                                    // Insert Clear
                                    m_objProcessDatabase.m_objDatabaseSendMessage.SetInsertHistoryEquipmentUpperLossTimeEvent(CDatabaseDefine.EEventValue.CLEAR);
                                    m_eEventValue = CDatabaseDefine.EEventValue.CLEAR;
                                    m_objPreviousDate = DateTime.Now;
                                }
                                // 딜레이 시간 초기화
