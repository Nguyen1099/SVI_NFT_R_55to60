...
namespace SVI_NFT_R
{
    public partial class InspStagePicker : CProcessAbstract
    {
        ...
        private const string ID = nameof(InspStagePicker);
        ...
        private bool SetNoCheckPick()
        {
            bool bReturn = false;
            m_objAlarmStructure.strAlarmFunction = MethodBase.GetCurrentMethod().Name;
            SetProcessLog("[START]", mLogType);
            SetProcessLog(string.Format("SetVacuum -> Cmd:{0}, SensorIgnore:{1}", CVacuumAbstract.EVacuumCommand.CMD_ON, CVacuumAbstract.ESensorCheck.IGNORE), mLogType);
            Vacuum.SetVacuumCommand(CVacuumAbstract.EVacuumCommand.CMD_ON, CVacuumAbstract.ESensorCheck.IGNORE);
            bReturn = true;
            SetProcessLog("[END] " + bReturn, mLogType);
            return bReturn;
        ...
        private bool SetNoCheckOff()
        {
            bool bReturn = false;
            m_objAlarmStructure.strAlarmFunction = MethodBase.GetCurrentMethod().Name;
            SetProcessLog("[START]", mLogType);
            SetProcessLog(string.Format("SetVacuum -> Cmd:{0}, SensorIgnore:{1}", CVacuumAbstract.EVacuumCommand.CMD_OFF, CVacuumAbstract.ESensorCheck.IGNORE), mLogType);
            Vacuum.SetVacuumCommand(CVacuumAbstract.EVacuumCommand.CMD_OFF, CVacuumAbstract.ESensorCheck.IGNORE);
            bReturn = true;
            SetProcessLog("[END] " + bReturn, mLogType);
            return bReturn;
            ...
            while (false == mbThreadExit)
            {
                if (ECommand.Idle != mCommand)
                {
                    ...
                    switch (mCommand)
                    {
                        ...
                        case ECommand.NoCheckPick:
                            if (true == SetNoCheckPick())
                            {
                                mStatus = EStatus.NoCheckPick;
                            }
                            break;

                        case ECommand.NoCheckOff:
                            if (true == SetNoCheckOff())
                            {
                                mStatus = EStatus.NoCheckOff;
                            }
                            break;
                    }
                        ...
                        if (EStatus.Unknown == mStatus)
                        {
                            ...
                            switch (iReturn)
                            {
                                ...
                                case (int)CDefine.EErrorProcess.ERROR_PROCESS_CONTINUE:
                                    ...
                                    else if (ECommand.NoCheckPick == mCommand)
                                    {
                                        mStatus = EStatus.NoCheckPick;
                                    }
                                    else if (ECommand.NoCheckOff == mCommand)
                                    {
                                        mStatus = EStatus.NoCheckOff;
                                    }
                                    break;
