...
namespace SVI_NFT_R
{
    public partial class InspStageMotorY : CProcessAbstract, IEmsHandle
    {
        ...
        public EEmsGroupFlags EmergencyStopGroup => EEmsGroupFlags.None;
        private const string ID = nameof(InspStageMotorY);
        ...
        public override bool Initialize(CDocument document, int position = 0, params object[] args)
        {
            ...
            do
            {
                mCommandMapping = new Dictionary<ECommand, MotorPositionSet<ECommand, EPosition>>()
                {
                    ...
                    [ECommand.GrabP1Position] = new MotorPositionSet<ECommand, EPosition>(ECommand.GrabP1Position, EPosition.GrabP1),
                    [ECommand.GrabP2Position] = new MotorPositionSet<ECommand, EPosition>(ECommand.GrabP2Position, EPosition.GrabP2),
                ...
                // 인터락 객체 생성
                m_objInterlock = new CProcessInterlock(new InspStageInterlock(m_objDocument));
            ...
            while (false == mbThreadExit)
            {
                if (ECommand.Idle != mCommand)
                {
                    ...
                    switch (mCommand)
                    {
                        ...
                        case ECommand.GrabP1Position:
                            if (SetMotorPosition(mCommandMapping[mCommand], mDefine.AlarmGrabP1PositionMoveFail) == true)
                            {
                                mStatus = EStatus.GrabP1Position;
                            }
                            break;

                        case ECommand.GrabP2Position:
                            if (SetMotorPosition(mCommandMapping[mCommand], mDefine.AlarmGrabP2PositionMoveFail) == true)
                            {
                                mStatus = EStatus.GrabP2Position;
                            }
                            break;
                        ...
                        if (EStatus.Unknown == mStatus)
                        {
                            ...
                            switch (iReturn)
                            {
                                ...
                                case (int)CDefine.EErrorProcess.ERROR_PROCESS_CONTINUE:
                                    ...
                                    else if (ECommand.GrabP1Position == mCommand)
                                    {
                                        mStatus = EStatus.GrabP1Position;
                                    }
                                    else if (ECommand.GrabP2Position == mCommand)
                                    {
                                        mStatus = EStatus.GrabP2Position;
                                    }
