...
namespace SVI_NFT_R
{
    public partial class OutRobotNachi : CProcessAbstract, IEmsHandle
    {
        ...
        private const string ID = nameof(OutRobotNachi);
        ...
        public bool IsAlignInterlock()
        {
            // 자동 모드일 경우
            if (Robot.Status.IsPlayBackModeEntry == true && Robot.Status.IsRunningU1 == true)
            {
                if (Robot.Signals.X.BB[Bit.EInput.P2_INTERLOCK].Value == false)
                {
                    return true;
                }
            }
                ...
                if (ECommand.Idle != mCommand)
                {
                    ...
                    switch (mCommand)
                    {
                        ...
                        case ECommand.LoadPermit:
                            if (true == SetProcessPermit(ERobotProcess.P1, ProcessMethodIndex, ProcessToolIndex, ProcessStageIndex, false))
