...
namespace SVI_NFT_R
{
    public partial class InRobot
    {
        ...
        private void SetPreOrderToProcessBegin(InRobotNachi.ECommand command, List<RobotSequenceItemSet> workOrders)
        {
            /// <![CDATA[
            /// - 나찌 로봇 택타임을 줄이기 위한 트릭임
        ...
        private bool SubPreOrderToProcessExit(InRobotNachi.ECommand command, List<RobotSequenceItemSet> workOrders = null, int currentIndex = -1)
        {
            /// <![CDATA[
            /// - 나찌 로봇 택타임을 줄이기 위한 트릭임
        ...
        private bool SubSequenceLoad(EMethod methodIndex, ETool toolIndex, EStage stageIndex, bool bCheckManualInterlock = false)
        {
            ...
            InRobotPicker[] selectRobotPickers = null;
            InShuttlePicker[] selectOtherPickers = null;
            var inShuttle = m_objDocument.m_objProcessMain.m_objProcessMotion.InShuttle;

            switch (toolIndex)
            {
                case ETool.Tool1:
                    selectRobotPickers = new InRobotPicker[] { Pickers[0] };
                    selectOtherPickers = new InShuttlePicker[] { inShuttle.Pickers[0] };
                    break;

                case ETool.Tool2:
                    selectRobotPickers = new InRobotPicker[] { Pickers[1] };
                    selectOtherPickers = new InShuttlePicker[] { inShuttle.Pickers[1] };
                    break;
            ...
            if (bCheckManualInterlock == true)
            {
                if (Nachi.m_objInterlock.CheckMotionClassInterlock(string.Empty, (int)InRobotNachi.ECommand.LoadPermit) == false)
                {
                    return false;
                }
            }
            ...
            {
                Array.ForEach(selectRobotPickers, picker => Nachi.SetMccLogItem(MccLogManager.InRobot.GET_MOVE_LD_POS[picker.CellPort.PositionIndex]));
            }
            Nachi.SetCommand(InRobotNachi.ECommand.LoadPermit);
            ...
            if (methodIndex == EMethod.Get)
            {
                if (bWriteMccLog == true)
                {
                    Array.ForEach(selectRobotPickers, picker => picker.SetMccLogItem(MccLogManager.InRobot.GET_VAC_ON[picker.CellPort.PositionIndex]));
                }
                Array.ForEach(selectRobotPickers, picker => picker.SetCommand(InRobotPicker.ECommand.Pick));
                ...
                if (bWriteMccLog == true)
                {
                    Array.ForEach(selectOtherPickers, picker => picker.SetMccLogItem(MccLogManager.InShuttle.PUT_VAC_OFF[picker.CellPort.PositionIndex]));
                }
                Array.ForEach(selectOtherPickers, picker => picker.SetCommand(InShuttlePicker.ECommand.Place));
            ...
            {
                Array.ForEach(selectOtherPickers, picker => picker.SetCommand(InShuttlePicker.ECommand.Pick));
                if (selectOtherPickers.WaitForEndProcess() == false)
                {
                    return false;
                }
                Array.ForEach(selectRobotPickers, picker => picker.SetCommand(InRobotPicker.ECommand.Place));
                if (selectRobotPickers.WaitForEndProcess() == false)
                {
                    return false;
                }
            ...
            mRobotCycleEnd = () =>
            {
                if (bWriteMccLog == true)
                {
                    Array.ForEach(selectRobotPickers, picker => Nachi.SetMccLogItem(MccLogManager.InRobot.GET_MOVE_LD_UP_POS[picker.CellPort.PositionIndex]));
                }
                Nachi.SetCommand(InRobotNachi.ECommand.LoadCycleEnd);
                if (Nachi.WaitForEndProcess() == false)
                {
                    return false;
                }
        ...
        private bool SubSequenceMcr(EMethod methodIndex, ETool toolIndex, EStage stageIndex, bool bCheckManualInterlock = false)
        {
            bool bWriteMccLog = methodIndex == EMethod.Put
                && bCheckManualInterlock == false;
            InRobotMcr[] selectRobotMcrs = null;

            switch (toolIndex)
            {
                case ETool.Tool1:
                    selectRobotMcrs = new InRobotMcr[] { Mcrs[0] };
                    break;

                case ETool.Tool2:
                    selectRobotMcrs = new InRobotMcr[] { Mcrs[1] };
                    break;

                case ETool.Tool12:
                    selectRobotMcrs = new InRobotMcr[] { Mcrs[0], Mcrs[1] };
                    break;
            ...
            if (bCheckManualInterlock == true)
            {
                if (Nachi.m_objInterlock.CheckMotionClassInterlock(string.Empty, (int)InRobotNachi.ECommand.McrPermit) == false)
                {
                    return false;
                }
            }
            ...
            if (bWriteMccLog == true)
            {
                Array.ForEach(selectRobotMcrs, mcr => Nachi.SetMccLogItem(MccLogManager.InRobot.MOVE_MCR_POS[mcr.CellPort.PositionIndex]));
            }
            Nachi.SetCommand(InRobotNachi.ECommand.McrPermit);
            if (Nachi.WaitForEndProcess() == false)
            {
                return false;
            }
            if (methodIndex == EMethod.Get)
            {
                //Array.ForEach(selectRobotMcrs, mcr => mcr.SetCommand(bCheckManualInterlock ? InRobotMcr.ECommand.GrabOneshot : InRobotMcr.ECommand.GrabSequence));
            }
            else if (methodIndex == EMethod.Put)
            {
                Array.ForEach(selectRobotMcrs, mcr =>
                {
                    if (bWriteMccLog == true)
                    {
                        mcr.SetMccLogItem(MccLogManager.InRobot.MCR_PROCESS[mcr.CellPort.PositionIndex]);
                    }
                    mcr.SetCommand(bCheckManualInterlock ? InRobotMcr.ECommand.GrabOneshot : InRobotMcr.ECommand.GrabSequence);
                });
            }
            if (selectRobotMcrs.WaitForEndProcess() == false)
            {
                return false;
            }

            mRobotCycleEnd = new Func<bool>(() =>
            {
                if (bWriteMccLog == true)
                {
                    Array.ForEach(selectRobotMcrs, mcr => Nachi.SetMccLogItem(MccLogManager.InRobot.MOVE_MCR_UP_POS[mcr.CellPort.PositionIndex]));
                }
                Nachi.SetCommand(InRobotNachi.ECommand.McrCycleEnd);
                if (Nachi.WaitForEndProcess() == false)
                {
                    return false;
                }
        ...
        private bool SubSequenceUnload(EMethod methodIndex, ETool toolIndex, EStage stageIndex, bool bCheckManualInterlock = false)
        {
            bool bWriteMccLog = methodIndex == EMethod.Put
                && bCheckManualInterlock == false;
            InRobotPicker[] selectRobotPickers = null;
            ...
            if (bCheckManualInterlock == true)
            {
                if (Nachi.m_objInterlock.CheckMotionClassInterlock(string.Empty, (int)InRobotNachi.ECommand.UnloadPermit) == false)
                {
                    return false;
                }
            }
            ...
            if (bWriteMccLog == true)
            {
                Array.ForEach(selectRobotPickers, picker => Nachi.SetMccLogItem(MccLogManager.InRobot.PUT_MOVE_ULD_POS[picker.CellPort.PositionIndex]));
            }
            Nachi.SetCommand(InRobotNachi.ECommand.UnloadPermit);
            if (Nachi.WaitForEndProcess() == false)
            {
                return false;
            }
            ...
            else if (methodIndex == EMethod.Put)
            {
                if (bWriteMccLog == true)
                {
                    //Array.ForEach(selectOtherPickers, picker => picker.SetMccLogItem(MccLogManager.InspStage.GET_VAC_ON[picker.CellPort.PositionIndex]));
                    MccLogManager.InspStage.GET_VAC_ON[otherPickerindex].WriteStart();
                }
                ...
                if (bWriteMccLog == true)
                {
                    MccLogManager.InspStage.GET_VAC_ON[otherPickerindex].WriteEnd();
                    Array.ForEach(selectRobotPickers, picker => picker.SetMccLogItem(MccLogManager.InRobot.PUT_VAC_OFF[picker.CellPort.PositionIndex]));
                }
                Array.ForEach(selectRobotPickers, picker => picker.SetCommand(InRobotPicker.ECommand.Place));
                if (selectRobotPickers.WaitForEndProcess() == false)
                {
            ...
            mRobotCycleEnd = () =>
            {
                if (bWriteMccLog == true)
                {
                    Array.ForEach(selectRobotPickers, picker => Nachi.SetMccLogItem(MccLogManager.InRobot.PUT_MOVE_ULD_UP_POS[picker.CellPort.PositionIndex]));
                }
                Nachi.SetCommand(InRobotNachi.ECommand.UnloadCycleEnd);
                if (Nachi.WaitForEndProcess() == false)
                {
                    return false;
                }
