...
namespace SVI_NFT_R
{
    public class OutShuttleInterlock : CProcessInterlockAbstract
    {
        /// <summary>
        /// 생성자 함수
        /// </summary>
        /// <param name="objDocument">CDocument의 객체</param>
        public OutShuttleInterlock(CDocument objDocument)
        {
        ...
        public override bool CheckMotionClassInterlock(string strMotorName, double dTargetPosition)
        {
            ...
            if (InternalCheckUpperEqInterfaceDoorOpen(m_objDocument.m_objProcessMain.m_objProcessMotion.LoadInterface) == false
                //|| InternalCheckLowerEqInterfaceDoorOpen(m_objDocument.m_objProcessMain.m_objProcessMotion.UnloadInterface) == false
                || InternalCheckUpperEqInterfaceEmergency(m_objDocument.m_objProcessMain.m_objProcessMotion.LoadInterface) == false
                //|| InternalCheckLowerEqInterfaceEmergency(m_objDocument.m_objProcessMain.m_objProcessMotion.UnloadInterface) == false
        ...
        public override bool CheckForcedInterlock(string strMotorName, bool bPositveDirection, Action callbackInterlockPreActionOrNull)
        {
            ...
            // 내부 키 체커 감지 확인
            // mặt sau không có khoá bên trong
            //if (m_objDocument.GetIOBit(CDeviceIODefine.EDigitalInput.X_FRONT_MAINT_KEY_CHECKER_DETECT) == false
            //    && m_objDocument.GetIOBit(CDeviceIODefine.EDigitalInput.X_REAR_MAINT_KEY_CHECKER_DETECT) == false)
            //{
            //    callbackInterlockPreActionOrNull?.Invoke();
            //    m_objDocument.SetMessage(CDefine.EAlarmType.ALARM_INTERLOCK, CAlarmDefine.EMessageList.EQUIPMENT_INSIDE_KEY_IS_NOT_DETECTED_PARAM1);
            //    return false;
            //}
